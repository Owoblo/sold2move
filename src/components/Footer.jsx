...
const Footer = () => {
  ...
  return (
    <footer className="px-6 py-20 border-t border-teal/20">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-2">
            ...
            >
              ...
              <p className="text-slate leading-relaxed mb-6">
                Your exclusive North American moving pipeline. The only moving data you can't afford to compete without.
              </p>
              <div className="flex space-x-4">
                {socialLinks.map((social) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="p-3 bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl border border-teal/20 rounded-xl hover:border-teal/40 transition-all duration-300 group"
                  >
                    <social.icon className="h-5 w-5 text-slate group-hover:text-teal transition-colors duration-300" />
                  </motion.button>
                ))}
              </div>
            </motion.div>
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.name}>
                    <Link
                      to={link.path}
                      className="text-slate hover:text-teal transition-colors duration-300 text-left"
                    >
                      {link.name}
                    </Link>
                  </li>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="pt-8 border-t border-teal/20 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0"
        >
