...
async function classify(runDir, { db = query, client } = {}) {
  ...
  while (client && !quotaBlocked && next < candidates.length) {
    ...
    } catch (e) { failed++; regions[row.acquisition_scope].failed++; if (e.status === 429) quotaBlocked = true; }
  }
  }
  ...
  fs.writeFileSync(path.join(runDir, 'ai-classification-summary.json'), JSON.stringify({ classified, failed, candidates: candidates.length, counts, regions, target_count: allTargets.length, screening_complete: allTargets.every(r => !r.classification_stale), quota_blocked: quotaBlocked }, null, 2));
}
