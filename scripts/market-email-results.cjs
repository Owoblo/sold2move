...
async function main() {
  ...
  await send({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:760px;color:#17324d"><h2>${label} Market Radar — Full Scrape</h2><p>This is the complete inventory delivery. A disappearance becomes reportable only after two successful misses; it does not prove a lease or sale.</p><p style="font-size:18px"><strong>Print-ready envelope pieces: ${postcardSummary.mail_eligible || 0}</strong></p><p>${postcardSummary.mail_eligible ? `Use the attached <b>${lane}-mail-eligible PDF</b> for the print shop. The CSV is supporting data only.` : 'No print-ready PDF was produced because no record passed the mailing rules.'}</p>${lane === 'commercial' ? `<p><strong>Direct relocation candidates requiring human review: ${relocationCandidates.length}</strong>. Generic property listings remain market intelligence only.</p>` : ''}<h3>Lifecycle</h3><table style="border-collapse:collapse">${tableRows(lifecycle.summary || {})}</table><h3>Acquisition coverage</h3><table style="border-collapse:collapse">${tableRows(regionCoverage)}</table><p>Every requested city and community—including zero-result and failed searches—is listed in the workbook’s <b>Market Coverage</b> sheet. Absence is never silently hidden.</p><h3>AI occupancy / furnishing</h3><table style="border-collapse:collapse">${tableRows(ai.counts || {})}</table><h3>Contact coverage</h3><table style="border-collapse:collapse">${tableRows(contacts)}</table><h3>Health checks</h3><table style="border-collapse:collapse">${tableRows(health)}</table><h3>Largest returned city inventories</h3><table style="border-collapse:collapse">${tableRows(cityTop)}</table><p style="color:#52697d">The attached XLSX is the audit workbook, not the print-shop file. It includes lifecycle, occupancy/furnishing state, AI confidence, evidence, contacts, source URLs, listing details and the complete requested-city coverage ledger.</p></div>`,
