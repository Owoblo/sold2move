...
async function main() {
  ...
  await send({
    ...
    attachments: [...['run-assessment.md', 'run-assessment.json', 'assessment-error.json'].filter(name => fs.existsSync(path.join(runDir,name))).map(name => ({ filename: name, content: fs.readFileSync(path.join(runDir,name)).toString('base64') })), ...rentalArtwork, { filename: `${lane}-full-market-report-${date}.xlsx`, content: fullWorkbook.toString('base64') }, ...(lane === 'commercial' ? [{
