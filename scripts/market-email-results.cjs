...
async function main() {
  ...
  const rentalArtwork = lane === 'rental' && fs.existsSync(path.join(runDir, 'postcards'))
    ? fs.readdirSync(path.join(runDir, 'postcards')).map(name => ({ filename: name, content: fs.readFileSync(path.join(runDir, 'postcards', name)).toString('base64') })) : [];
