...
async function enrich(runDir,{db=query,request=api}={}){
 ...
 await Promise.all([worker(),worker(),worker()]);fs.writeFileSync(file,JSON.stringify(rows,null,2));fs.writeFileSync(path.join(runDir,'detail-summary.json'),JSON.stringify({targets:targets.length,reused,enriched,failures},null,2));
}
