...
async function buildRentalReport(runDir) {
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:600px;color:#1a1a1a">
    ...
    ${cost?section('Scrape Cost')+`<p>Latest saved Apify cost snapshot (${escape(cost.generatedAt || "date unavailable")}): <b>$${Number(cost.knownTotalUsd).toFixed(2)} USD</b>${cost.complete?'':' (partial; some costs pending)'}. This saved figure is cumulative. The separate cost email sent after processing contains the final current-run total. OpenAI, printing and postage are separate.</p>`:''}
