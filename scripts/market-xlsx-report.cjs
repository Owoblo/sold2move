...
const RENTAL_COLUMNS = [
  ...
  ['Current Occupancy', row => row.current_occupancy || 'unknown'],
  ['Offered Furnishing', row => row.occupancy_state || 'unknown'],
  ...
  ['Address', row => [row.mailing_street || row.street_address, row.city, row.province, row.postal_code].filter(Boolean).join(', ')],
  ['City', row => row.city],
