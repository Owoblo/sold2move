...
Fresh Zillow details are fetched only for new/relisted unit-specific or single-home candidates, across every region, reusing details or completed classification already saved for this observation. AI classifies all new/relisted candidates with identifiable units/homes, cycling across regions with three workers. There is no first-150 cutoff. It does not rescan the whole active inventory each week. It separately records current occupancy and offered furnishing. Reusable classifications require the same description/photos/unit fingerprint and classifier version and must be under 30 days old. Reappearances require classification from this observation or later; resuming the same observation reuses completed work. Missing/failed/low-confidence evidence stays in review.
...
Resuming a completed workflow produces a supplemental batch keyed to the source workflow run, excluding all addresses reserved in prior batches. The owner email shows per-region candidates, screened/reused, pending and additional postcard counts. Disappearance tracking is explicitly labeled as not a postcard list.

Rental owner email uses the residential pipeline layout: summary, highlighted attached postcard count, six-region screening coverage, recipient-city breakdown, cost and attachments. Disappearances stay in this single report and workbook; they do not generate a separate rental action email or another mailing. Zero-recipient reports omit empty CSV/PDF attachments. `report_only=true` with `source_run_id` restores and emails saved results without database changes, acquisition, classification or batch generation.

Address audit: fetch details for all fresh new/relisted source records before requiring a specific unit or whole-home classification. Preserve explicit source units and recover unambiguous advertised MAIN/UPPER/LOWER/BSMT units from listing descriptions; room locations alone are insufficient. Address corrections keep original recipient identity and do not create another prospect. Previously generated source listings are suppressed even if their unit-qualified mailing key improves.

Follow-up review: previously batched listings that disappear can enter a review shortlist after 21 days, with 28 days preferred. Generated batch dates are labelled unconfirmed mailing dates. No automatic second print occurs: confirm actual first mailing, outgoing occupancy and move timing. Plan arrival 7–14 days before an expected move; 1st/15th are unverified calendar assumptions overridden by actual dates.
